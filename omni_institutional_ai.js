...
const DAILY_PIPELINE_MS = Number(process.env.DAILY_PIPELINE_MS||24*60*60*1000); // daily
...
async function AIDecision(symbol,tf){
  ...
  if(c.length<50) return {direction:null,quality:0};
  const decisions = [];
...
async function dailyExtraction(symbols){
  for(const symbol of symbols){
    for(const tf of [...SWING_TF,...SCALP_TF]){
      ...
      WATCH.set(symbol,{symbol,tf,indicators,news,correlation:corr,riskMode:risk,strategies,ai});
    }
  }
}
...
async function analyze(symbol,chat=TELEGRAM_CHAT_ID,mode="swing",tfs=null){
  tfs=tfs|| (mode==="swing"?SWING_TF:SCALP_TF);
...
/* ================= DAILY PIPELINE ================= */
(async()=>{
  const symbols = ["BTCUSDT","ETHUSDT","BNBUSDT","SOLUSDT"]; // add all your market symbols
  while(true){
