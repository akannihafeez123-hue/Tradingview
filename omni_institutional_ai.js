...
async function analyze(symbol,chat=TELEGRAM_CHAT_ID,mode="swing",tfs=null){
  ...
  if(confidence < THRESHOLD_WATCH){
    send(chat, `OMNI AI SIGNAL SYMBOL: ${symbol} MODE: ${mode} STATUS: WAIT | Confidence ${(confidence*100).toFixed(2)}%`);
    return;
  }
  ...
  send(chat,
    `OMNI AI SIGNAL SYMBOL: ${symbol} MODE: ${mode} DIRECTION: ${direction} ENTRY: ${entryPrice.toFixed(4)} CONFIDENCE: ${(confidence*100).toFixed(2)}% TP: ${tp.toFixed(4)}, SL: ${sl.toFixed(4)}\n${lines.join("\n")}`
  );
}
