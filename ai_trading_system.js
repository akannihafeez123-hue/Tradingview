...
class RealLogger {
  ...
  tradeSignal(symbol, direction, entry, sl, tp, confidence) {
    const signal = {
      timestamp: Date.now(),
      symbol,
      direction,
    ...
    const message = `📈 ${direction} ${symbol} @ $${entry} | SL: $${sl} | TP: $${tp.join('/')} | Conf: ${Math.round(confidence * 100)}%`;
...
class RealMarketData {
  ...
  async fetchFromAlpaca(symbol, timeframe, limit) {
    ...
    try {
      ...
      if (response.json && response.json.bars) {
        return {
          source: 'alpaca',
          data: response.json.bars.map(bar => ({
  ...
  async fetchFromYahoo(symbol, timeframe, limit) {
    try {
      ...
      if (response.json && response.json.chart && response.json.chart.result) {
        ...
        return { source: 'yahoo', data: data.slice(-limit) };
      }
    } catch (error) {
  ...
  async fetchFromCoinGecko(symbol, timeframe, limit) {
    try {
      ...
      if (response.json && Array.isArray(response.json)) {
        return {
          source: 'coingecko',
          data: response.json.slice(-limit).map(candle => ({
            timestamp: candle[0],
            open: candle[1],
            high: candle[2],
            low: candle[3],
  ...
  async fetchFromBybit(symbol, timeframe, limit) {
    ...
    try {
      ...
      if (response.json && response.json.result && response.json.result.list) {
        return {
          source: 'bybit',
          data: response.json.result.list.reverse().map(candle => ({
  ...
  async fetchFromTwelveData(symbol, timeframe, limit) {
    ...
    try {
      ...
      if (response.json && response.json.values) {
        return {
          source: 'twelvedata',
          data: response.json.values.map(item => ({
  ...
  async fetchFromPolygon(symbol, timeframe, limit) {
    ...
    try {
      ...
      if (response.json && response.json.results) {
        return {
          source: 'polygon',
          data: response.json.results.map(item => ({
            timestamp: item.t,
            open: item.o,
            high: item.h,
            low: item.l,
  ...
  async fetchFromFinancialModeling(symbol, timeframe, limit) {
    try {
      ...
      if (response.json && Array.isArray(response.json)) {
        return {
          source: 'fmp',
          data: response.json.slice(0, limit).map(item => ({
  ...
  async fetchFromMarketStack(symbol, timeframe, limit) {
    try {
      ...
      if (response.json && response.json.data) {
        return {
          source: 'marketstack',
          data: response.json.data.map(item => ({
            timestamp: new Date(item.date).getTime(),
            open: item.open,
            high: item.high,
            low: item.low,
  ...
  async getMarketStatus() {
    try {
      ...
      // Check crypto market (always open)
  ...
  getNextMarketOpen(now) {
    ...
    if (day === 5 && hour >= 16) { // Friday after close
      next.setDate(next.getDate() + 3); // Monday
      ...
      next.setHours(9, 30, 0, 0);
    } else if (hour >= 16) { // After close on weekday
      next.setDate(next.getDate() + 1);
      next.setHours(9, 30, 0, 0);
    } else if (hour < 9 || (hour === 9 && next.getMinutes() < 30)) { // Before open
      next.setHours(9, 30, 0, 0);
    } else {
...
class RealTechnicalAnalysis {
  ...
  // REAL INDICATOR CALCULATIONS (No simulations)
  ...
  findSupportLevels(closes, lows) {
    ...
    for (let i = 10; i < lows.length - 10; i++) {
      ...
      if (currentLow <= leftMin && currentLow <= rightMin) {
        // Check if this level is significant
  ...
  isMarketTrending(closes, period = 50) {
    ...
    // Check if price is consistently above/below both SMAs
  ...
  isMarketVolatile(closes, period = 20) {
    ...
    return volatility > 0.4; // 40% annualized volatility
  }
...
class RealAIStrategyEngine {
  ...
  analyzeMarket(priceData, indicators) {
    ...
    // 5. Technical Levels
    analysis.conditions.key_level_proximity = this.isNearKeyLevel(currentPrice, indicators);
    ...
    // 7. Risk Analysis
    analysis.conditions.low_correlation = this.hasLowCorrelation(closes); // Simplified
  ...
  selectOptimalStrategy(marketAnalysis, confidenceThreshold = MIN_CONFIDENCE_THRESHOLD) {
  ...
  // REAL CONDITION DETECTION METHODS
  ...
  hasLowCorrelation(closes) {
    ...
    return stdDev / Math.abs(mean) > 2; // High volatility relative to mean change
  }
...
class RealTradingEngine {
  ...
  async analyzeSymbol(symbol, timeframe = '1h') {
    try {
      ...
      logger.info(`✅ Analysis complete: ${strategySelection.name} (${Math.round(strategySelection.confidence * 100)}% confidence)`);
  ...
  async generateTradeSignal(symbol, priceData, indicators, marketAnalysis, strategySelection) {
    ...
    let direction = 'NEUTRAL';
    let confidence = strategySelection.confidence;
    ...
    // If no direction found, return neutral
    ...
    return {
      ...
      // Timing
      timestamp: Date.now(),
      expiry: Date.now() + (24 * 60 * 60 * 1000), // 24 hour expiry
      timeframe: '1H-4H'
    };
  }
  
  async generateQuantumSignal(closes, highs, lows, indicators, marketAnalysis) {
    // Quantum strategy: Multi-timeframe institutional analysis
  ...
  async generateMomentumSignal(closes, indicators, marketAnalysis) {
    // Momentum strategy: Follow strong momentum with volume confirmation
  ...
  async generateMeanReversionSignal(closes, indicators, marketAnalysis) {
    // Mean reversion strategy: Trade overextensions
  ...
  detectInstitutionalPatterns(closes, highs, lows, indicators) {
    // Simplified institutional pattern detection
    // In production, this would use more advanced pattern recognition
  ...
  checkBearishDivergence(closes, indicators) {
    // Simplified bearish divergence check
    if (closes.length < 30) return false;
  ...
  calculatePositionSize(currentPrice, atr, confidence, direction) {
    ...
    const riskPerUnit = atr * 1.5; // Stop loss at 1.5 ATR
    ...
    const maxPositionPercentage = 20; // Max 20% of account
  ...
  calculateTakeProfit(currentPrice, atr, direction, supportLevels, resistanceLevels) {
    ...
    if (direction === 'LONG') {
      ...
      // Second target: 1:2 risk/reward or next resistance
      if (resistanceLevels && resistanceLevels.length > 0) {
      ...
      // Third target: 1:3 risk/reward
      takeProfitLevels.push(currentPrice + (atr * 4.5));
    } else if (direction === 'SHORT') {
      // First target: 1:1 risk/reward
      takeProfitLevels.push(currentPrice - (atr * 1.5));
      
      // Second target: 1:2 risk/reward or next support
      if (supportLevels && supportLevels.length > 0) {
  ...
  async getMarketOverview(symbols = ['BTCUSDT', 'ETHUSDT', 'AAPL', 'EURUSD', 'XAUUSD']) {
    ...
    for (const symbol of symbols) {
      try {
        ...
        overview.summary.averageConfidence += analysis.confidence || 0;
    ...
    overview.summary.averageConfidence = overview.markets.length > 0 ? 
      overview.summary.averageConfidence / overview.markets.length : 0;
...
class TelegramWebhookHandler {
  ...
  async setWebhook() {
    if (!this.webhookUrl) {
      logger.warn('TELEGRAM_WEBHOOK_URL not set. Webhook will not be configured.');
      return false;
    }
    
    try {
      const data = new URLSearchParams();
      data.append('url', this.webhookUrl);
      ...
      if (result.ok) {
        logger.info(`✅ Telegram webhook set: ${this.webhookUrl}`);
...
<b>Reason:</b> ${signal.reason || 'Insufficient confidence'}

<b>AI Strategy:</b> ${strategy.name} ${strategy.emoji}
<b>Confidence:</b> ${Math.round(strategy.confidence * 100)}% ${strategy.confidenceLevel === 'HIGH' ? '✅' : '⚠️'}
...
🔄 <b>Mean Reversion Pro</b> - Statistical arbitrage
...
<b>Webhook:</b> ${this.webhookUrl ? '🟢 CONFIGURED' : '🔴 NOT SET'}

<b>🎯 Trading Parameters:</b>
Account Equity: $${ACCOUNT_EQUITY}
Risk per Trade: ${DEFAULT_RISK_PCT}%
Min Confidence: ${(MIN_CONFIDENCE_THRESHOLD * 100).toFixed(0)}%
  ...
  async sendMarketOverview(chatId) {
    ...
    try {
      ...
      overview.markets.forEach(market => {
        ...
        message += `RSI: ${market.rsi.toFixed(1)} | Conf: ${Math.round(market.confidence * 100)}%\n`;
        message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
      });
      ...
      message += `Avg Confidence: ${Math.round(overview.summary.averageConfidence * 100)}%\n\n`;
...
class AI_Trading_System {
  ...
  async initialize() {
    try {
      ...
      this.telegramHandler = new TelegramWebhookHandler();
      
      // Setup webhook if URL provided
      if (TELEGRAM_WEBHOOK_URL) {
        logger.info('🔧 Setting up Telegram webhook...');
        const webhookSet = await this.telegramHandler.setWebhook();
        
        if (webhookSet) {
          logger.info('✅ Telegram webhook configured successfully!');
        } else {
          logger.warn('⚠️  Webhook setup failed. You may need to set it manually.');
          logger.info('💡 Manual command:');
          logger.info(`curl -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/setWebhook" \\`);
          logger.info(`     -d "url=${TELEGRAM_WEBHOOK_URL}"`);
        }
      } else {
  ...
  startHttpServer() {
    ...
    server.listen(PORT, () => {
      ...
      logger.info(`📱 Telegram webhook: POST http://localhost:${PORT}/telegram`);
  ...
  async handleHealth(req, res) {
    ...
    const health = {
      ...
      apis: {
        telegramWebhook: !!TELEGRAM_WEBHOOK_URL,
  ...
  async handleTelegramWebhook(req, res) {
    ...
    req.on('end', async () => {
      try {
        ...
        this.telegramHandler.handleWebhookUpdate(update).catch(error => {
  ...
  async handleTradingViewWebhook(req, res) {
    ...
    req.on('end', async () => {
      try {
        ...
        const { symbol, alertType, price } = alert;
        
        if (symbol && this.telegramHandler) {
          const message = `🚨 TradingView Alert: ${symbol} - ${alertType || 'signal'} @ $${price || 'N/A'}`;
          await this.telegramHandler.sendMessage(this.telegramHandler.chatId, message);
          
          // Trigger analysis
          setTimeout(async () => {
            try {
              const analysis = await this.tradingEngine.analyzeSymbol(symbol);
              await this.telegramHandler.sendAnalysisResults(this.telegramHandler.chatId, analysis);
            } catch (error) {
