...
function atrConfirm(entry, price, atr, dir, factor = 0.15) {
  if (!atr || atr === 0) return true; // Skip confirmation if no ATR
...
function htfGate(macro, ltfDirection) {
  if (!macro || macro.regime === "Neutral") return true; // Allow if neutral
...
/* ================= CALCULATE TP/SL ================= */
function calculateTPSL(entry, direction, atr, riskReward = 2) {
  if (!atr || atr === 0) {
    // Fallback if no ATR
    const defaultRisk = entry * 0.02; // 2% risk
    if (direction === "BUY") {
      return {
        takeProfit: entry + (defaultRisk * riskReward),
        stopLoss: entry - defaultRisk
      };
    } else {
      return {
        takeProfit: entry - (defaultRisk * riskReward),
  ...
  if (direction === "BUY") {
    return {
      takeProfit: entry + (atr * riskReward),
      stopLoss: entry - atr
    };
  } else {
    return {
...
async function AIDecision(symbol, tf, macro, btcDom) {
  ...
  if (tickDir !== "NEUTRAL" && tickDir !== candleDir) {
    // If tick bias conflicts, reduce confidence
    // but don't completely reject
  }
  
  const atr = ATR(c);
  if (!atrConfirm(entry, entry, atr, candleDir)) {
    // Still proceed but note weak confirmation
  }

  const sess = sessionBias();

  for (let i = 0; i < AI_COUNT; i++) {
    const volZ = ATR_Z(c), atrVal = ATR(c), delta = VolumeDelta(c);
    ...
    let qualityScore = 30 * Math.min(1, Math.abs(volZ) / 2) +
    ...
    qualityScore = Math.min(100, Math.max(0, qualityScore * (Math.abs(correlation) > 0.8 ? 0.8 : 1)));
    qualityScore *= sess.weight; // Apply session bias

    let direction = (bos && c.at(-1).c > c.at(-2).c) || (absorption && c.at(-1).c > c.at(-2).c) ? "BUY" : "SELL";
    
    // If we have a clear candle bias, use it
    if (candleDir !== "NEUTRAL") {
      direction = candleDir;
    }

    if (macro) {
      if (macro.regime === "Bull" && direction === "SELL") {
        qualityScore *= 0.7; // Reduce confidence for counter-trend
      }
      if (macro.regime === "Bear" && direction === "BUY") {
        qualityScore *= 0.7; // Reduce confidence for counter-trend
      }
    }
    ...
    if (typeof btcDom === "number") {
      if (btcDom > 60 && direction === "BUY") {
        qualityScore *= 0.8; // Reduce confidence for BUY when BTC dominance high
      }
      if (btcDom < 40 && direction === "SELL") {
        qualityScore *= 0.8; // Reduce confidence for SELL when BTC dominance low
      }
    }
  ...
  // Calculate TP/SL
  const { takeProfit, stopLoss } = calculateTPSL(entry, dir, ATR(c), 2);

  return {
    direction: dir,
    quality: avgQuality,
    confidence: avgQuality > 70 ? "HIGH" : avgQuality > 50 ? "MEDIUM" : "LOW",
    strategies: [...new Set(decisions.flatMap(x => x.strategies))], // Remove duplicates
    entry: entry,
    takeProfit: takeProfit,
    stopLoss: stopLoss,
    atr: ATR(c),
    session: sess.name,
    riskRewardRatio: Math.abs((takeProfit - entry) / (entry - stopLoss)).toFixed(2)
  };
}

/* ================= ANALYZE COMMAND ================= */
async function analyzeCommand(symbol, tf = "1h") {
  const btcDom = await BTCDominance();
  const macro = await macro2Y(symbol);
  const decision = await AIDecision(symbol, tf, macro, btcDom);
  
  if (!decision || decision.direction === null) {
    return `❌ No valid signal for ${symbol} on ${tf}\nMarket conditions not favorable or insufficient data.`;
  }
  
  return `🔍 ${symbol} Analysis (${tf})\n\n` +
         `📈 Direction: ${decision.direction === "BUY" ? "🟢 BUY" : "🔴 SELL"}\n` +
         `🎯 Entry: $${decision.entry.toFixed(2)}\n` +
         `📊 AI Confidence: ${decision.confidence} (${decision.quality.toFixed(1)}%)\n` +
         `🎯 Take Profit: $${decision.takeProfit.toFixed(2)}\n` +
         `🛑 Stop Loss: $${decision.stopLoss.toFixed(2)}\n` +
         `📈 Risk/Reward: 1:${decision.riskRewardRatio}\n` +
         `📊 ATR: ${decision.atr.toFixed(2)}\n` +
         `🌍 Session: ${decision.session}\n` +
         `⏰ Time: ${new Date().toLocaleTimeString()}\n\n` +
         `📋 Strategies: ${decision.strategies.slice(0, 3).join(", ")}${decision.strategies.length > 3 ? "..." : ""}`;
}
...
async function handleTelegramCommand(msg) {
  ...
  const now = Date.now();
  const lastCommand = COMMAND_COOLDOWN.get(chatId) || 0;
  if (now - lastCommand < 2000) { // 2 second cooldown
    return; // Ignore rapid commands
  }
  COMMAND_COOLDOWN.set(chatId, now);
  ...
  if (cmd === "/list") {
    if (!WATCH.size) return send(chatId, "📭 Watchlist is empty.");
    const list = Array.from(WATCH.entries())
      .map(([symbol, config]) => `${symbol} (${config.tf})`)
      .join("\n");
    return send(chatId, `📋 Watchlist:\n${list}`);
  }

  if (cmd === "/swing") {
    const symbol = args[1]?.toUpperCase();
    if (!symbol) return send(chatId, "Usage: /swing SYMBOL");
    
    await send(chatId, `⏳ Analyzing ${symbol} for swing opportunities...`);
    ...
    for (const tf of SWING_TF) {
      const decision = await AIDecision(symbol, tf, macro, btcDom);
      if (decision && decision.direction && decision.quality > 30) {
        const signal = decision.direction === "BUY" ? "🟢" : "🔴";
        results.push(`${signal} ${tf}: ${decision.direction} | Confidence: ${decision.confidence} (${decision.quality.toFixed(1)}%) | Entry: $${decision.entry.toFixed(2)} | TP: $${decision.takeProfit.toFixed(2)} | SL: $${decision.stopLoss.toFixed(2)}`);
      } else {
        results.push(`⚪ ${tf}: No clear signal`);
      }
    }

    const summary = results.join("\n");
    return send(chatId, `📊 ${symbol} SWING Analysis:\n\n${summary}`);
  }

  if (cmd === "/scalp") {
    const symbol = args[1]?.toUpperCase();
    if (!symbol) return send(chatId, "Usage: /scalp SYMBOL");
    
    await send(chatId, `⏳ Analyzing ${symbol} for scalp opportunities...`);
    ...
    for (const tf of SCALP_TF) {
      ...
      if (decision && decision.direction && decision.quality > 30) {
        const signal = decision.direction === "BUY" ? "🟢" : "🔴";
        results.push(`${signal} ${tf}: ${decision.direction} | Conf: ${decision.confidence} (${decision.quality.toFixed(1)}%) | Entry: $${decision.entry.toFixed(2)} | TP: $${decision.takeProfit.toFixed(2)} | SL: $${decision.stopLoss.toFixed(2)}`);
      }
    }

    if (results.length === 0) {
      return send(chatId, `❌ No scalp opportunities found for ${symbol}`);
    }
    
    const summary = results.slice(0, 5).join("\n"); // Show only top 5
    return send(chatId, `📊 ${symbol} SCALP Analysis (Top Signals):\n\n${summary}`);
  }
  ...
  if (cmd === "/analyze") {
    ...
    await send(chatId, `🔍 Analyzing ${symbol} on ${tf}...`);
    const analysis = await analyzeCommand(symbol, tf);
    return send(chatId, analysis);
  }
  ...
  if (cmd === "/pipeline") {
    ...
    await send(chatId, `📊 Extracting pipeline data for ${symbol}...`);
  ...
  if (cmd === "/help" || cmd === "/start") {
    const helpText = `
🤖 *OMNI INSTITUTIONAL AI BOT*

*Available Commands:*

📊 *Analysis Commands*
/analyze [SYMBOL] [TIMEFRAME] - Detailed single analysis
/swing SYMBOL - Swing trading analysis (1d, 2d, 1w, 1M)
/scalp SYMBOL - Scalp trading analysis (5m to 4h)
...
📈 *Advanced Features*
/pipeline SYMBOL - Extract and store market data
...
*Note:* Bot monitors watchlist and sends alerts automatically.
    `;
...
async function main() {
  ...
  console.log("📊 Version: Unified Edition");
  ...
  console.log("🤖 Bot is starting Telegram polling...");
  
  // Send startup message
  try {
    await send(TELEGRAM_CHAT_ID, "🤖 OMNI INSTITUTIONAL AI is now online!\nType /help for commands.");
  } catch (e) {
    ...
    try {
      ...
      if (updates?.result) {
        for (const u of updates.result) {
          if (u.message) {
            console.log(`📩 Received command from ${u.message.chat.id}: ${u.message.text}`);
  ...
  setInterval(async () => {
    ...
    for (const [symbol, config] of WATCH.entries()) {
      ...
      if (now - last >= WATCH_INTERVAL_MS) {
        ...
        if (decision && decision.direction && decision.quality > 65) {
          const signal = decision.direction === "BUY" ? "🟢" : "🔴";
          const msg = `${signal} *${symbol} Alert (${config.tf})*\n\n` +
            `*Direction:* ${decision.direction}\n` +
            `*Confidence:* ${decision.confidence} (${decision.quality.toFixed(1)}%)\n` +
            `*Entry:* $${decision.entry.toFixed(2)}\n` +
            `*Take Profit:* $${decision.takeProfit.toFixed(2)}\n` +
            `*Stop Loss:* $${decision.stopLoss.toFixed(2)}\n` +
            `*Risk/Reward:* 1:${decision.riskRewardRatio}\n` +
            `*Session:* ${decision.session}\n` +
            `*Time:* ${new Date().toLocaleTimeString()}\n\n` +
            `_Auto-generated signal from Omni AI_`;
          
          await send(TELEGRAM_CHAT_ID, msg);
          LAST_EXECUTION.set(symbol, now);
          console.log(`🚨 Sent alert for ${symbol}`);
        }
      }
    }
  ...
  setInterval(async () => {
    ...
    console.log("📊 Running pipeline extraction...");
    for (const symbol of WATCH.keys()) {
      await extractPipeline(symbol);
    }
    console.log("✅ Pipeline extraction completed at", new Date().toLocaleTimeString());
  }, DAILY_PIPELINE_MS);
}
