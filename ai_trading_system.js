...
class RealMarketData {
  ...
  async getCurrentPrice(symbol) {
    ...
    try {
      ...
      if (symbolType === 'crypto') {
        const response = await http.get(`https://api.binance.com/api/v3/ticker/price?symbol=${symbol.replace('/', '')}`, { timeout: 5000 });
        if (response.json) {
          return parseFloat(response.json.price);
        }
      } else if (symbolType === 'stock') {
        const response = await http.get(`https://finnhub.io/api/v1/quote?symbol=${symbol}&token=${FINNHUB_API_KEY || 'demo'}`, { timeout: 5000 });
        if (response.json) {
...
// ==================== TELEGRAM BOT HANDLER (WITH CONFLICT FIX) ====================
class TelegramBotHandler {
  constructor() {
    ...
    logger.info(`Telegram Bot Handler initialized (Polling Mode) - Instance: ${this.instanceId}`);
  }
    ...
    try {
      ...
      if (result.ok) {
        ...
        if (result.error_code === 409) {
          logger.warn(`⚠️ Another bot instance detected. This instance will work in API-only mode.`);
          logger.warn(`   You can stop other instances or use webhook mode instead.`);
          this.isRunning = false;
          return [];
        }
      ...
      if (this.retryCount >= this.maxRetries) {
        logger.error(`Max retries reached for getUpdates. Switching to API-only mode.`);
        this.isRunning = false;
        return [];
      }
      
      logger.warn(`Get updates attempt ${this.retryCount}/${this.maxRetries} failed: ${error.message}`);
      
      // Exponential backoff
      const backoffTime = Math.min(1000 * Math.pow(2, this.retryCount), 30000);
      await http.sleep(backoffTime);
  ...
  async startPolling() {
    ...
    this.isRunning = true;
    logger.info(`Starting Telegram polling (Instance: ${this.instanceId})...`);
        ...
        if (updates && updates.length > 0) {
          for (const update of updates) {
            ...
            this.handleUpdate(update).catch(error => {
              logger.error(`Update handler error: ${error.message}`);
            });
          }
        }
        
        await http.sleep(this.pollingInterval);
      } catch (error) {
        logger.error(`Polling loop error: ${error.message}`);
        
        if (error.message.includes('409') || error.message.includes('Conflict')) {
          logger.warn('⚠️ Bot conflict detected. This instance will continue in API-only mode.');
          this.isRunning = false;
          break;
        }
        
        await http.sleep(5000); // Wait 5 seconds on error
      }
    }
    
    if (!this.isRunning) {
      logger.info('Telegram polling stopped. Bot running in API-only mode.');
    }
  }
  ...
  async handleUpdate(update) {
    ...
    try {
      if (text.startsWith('/analyze ')) {
        ...
        await this.sendMessage(chatId, 'Bot polling stopped. System running in API-only mode.');
        this.stopPolling();
      } else if (text === '/restart') {
        await this.sendMessage(chatId, 'Restarting polling...');
        this.isRunning = false;
...
<b>Instance:</b> ${this.instanceId}
<b>System:</b> ${this.isRunning ? '🟢 OPERATIONAL' : '🟡 API-ONLY MODE'}
<b>Mode:</b> ${MODE.toUpperCase()}
<b>AI Engine:</b> 🟢 ACTIVE
<b>Telegram:</b> ${this.isRunning ? '🟢 POLLING' : '🟡 API-ONLY'}
...
${!this.isRunning ? '\n⚠️ Note: Running in API-only mode due to another bot instance' : ''}
      `;
...
class AI_Trading_System {
  ...
  async initialize() {
    try {
      ...
      logger.info('• Telegram polling with conflict handling');
      logger.info('');
      ...
      logger.info('🕐 Startup Time: ' + new Date().toLocaleString());
      logger.info('');
      ...
      // Start Telegram polling in background (will handle conflicts gracefully)
      setTimeout(() => {
        this.telegramBot.startPolling().catch(error => {
          logger.error(`Failed to start Telegram polling: ${error.message}`);
      ...
      logger.info('🤖 Telegram: Will attempt polling (handles conflicts gracefully)');
  ...
  async handleHealth(req, res) {
    ...
    const health = {
      ...
      system: {
        ...
        telegramPolling: this.telegramBot?.isRunning || false
      },
      
      market: status,
      
      apis: {
        telegram: this.telegramBot?.isRunning ? 'POLLING' : 'API_ONLY',
  ...
  async handleTest(req, res) {
    const test = {
      ...
      features: [
        ...
        'Telegram polling with conflict handling'
      ],
      endpoints: {
        health: 'GET /health',
...
<body>
    <div class="container">
        ...
        <div class="status">
            ...
            <p>Telegram: ${this.telegramBot?.isRunning ? '🟢 POLLING' : '🟡 API-ONLY'}</p>
            <p>Port: ${PORT}</p>
        </div>
...
curl -X POST http://localhost:${PORT}/api/analyze \\
  ...
  -d '{"symbol": "BTCUSDT", "timeframe": "1h"}'
        ...
        <div class="status">
            ...
            <p>• Telegram handles conflicts gracefully</p>
            <p>• System works in API-only mode if needed</p>
