...
class LiquidationRiskAnalyzer {
  ...
  parseLiquidationData(liquidations, symbol) {
    ...
    const longLiqVolume = longLiquidations.reduce((sum, l) => sum + parseFloat(l.size), 0);
    const shortLiqVolume = shortLiquidations.reduce((sum, l) => sum + parseFloat(l.size), 0);
  ...
  findLiquidationClusters(liquidations) {
    ...
    const clusters = [];
    liquidations.sort((a, b) => a.ts - b.ts);
    ...
    for (const liq of liquidations) {
      if (currentCluster.length === 0) {
        ...
        if (liq.ts - lastLiq.ts <= TIME_WINDOW) {
          currentCluster.push(liq);
        } else {
          if (currentCluster.length >= 3) {
            clusters.push({
              start: currentCluster[0].ts,
              end: currentCluster[currentCluster.length - 1].ts,
              count: currentCluster.length,
              totalVolume: currentCluster.reduce((sum, l) => sum + parseFloat(l.size), 0),
              avgPrice: mean(currentCluster.map(l => parseFloat(l.price)))
            });
          }
          currentCluster = [liq];
        }
  ...
  estimateLiquidationLevels(symbol, liquidations) {
    const priceLevels = liquidations.map(l => parseFloat(l.price));
