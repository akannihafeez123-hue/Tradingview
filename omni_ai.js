...
const ACCOUNT_RISK_PERCENT = parseFloat(process.env.ACCOUNT_RISK_PERCENT || "1.0");
...
function tickBias(symbol) {
  ...
  // Consider velocity for stronger bias
...
function candleBias(c) {
  ...
  // Stronger bias if consecutive same direction
...
const ParkinsonVol = c => Math.sqrt(mean(c.map(x => Math.log(x.h / x.l) ** 2)) / (4 * Math.log(2)));
...
const EWMA = (c, l = 0.94) => {
  ...
  return Math.sqrt(v * 252); // Annualized
};
...
const LiquidityVoid = c => {
  ...
  const currentRange = c.at(-1).h - c.at(-1).l;
...
const StopHuntProb = c => {
  ...
  return Math.min(1, Math.abs(c.at(-1).c - c.at(-20).c) / (ATR(c) || 1));
};

const IcebergProxy = c => {
  if (c.length < 20) return false;
  return c.at(-1).v > mean(c.slice(-20).map(x => x.v)) * 4;
};
...
const RangeState = c => {
  ...
  const priceChange = Math.abs(c.at(-1).c - c.at(-20).c);
...
const TimeSymmetry = c => {
  ...
  return mean(c.slice(-10).map((x, i) => x.c - c[i].c));
};
...
function calculateRiskParameters(entry, direction, atr, quality, timeframe, volatility) {
  // Timeframe-based ATR multipliers
  const tfMultipliers = {
...
async function BTCDominance() {
  try {
    ...
    console.warn("BTC Dominance fetch failed, using default 50%");
    return 50; 
  }
}
...
async function CorrelationFilter(symbol) {
  try {
    ...
    const btcClose = btc.slice(-20).map(x => x.c);
    const sClose = s.slice(-20).map(x => x.c);
...
function supportResistance(c, levels = 3) {
  if (c.length < 50) return { support: [], resistance: [] };
  ...
  return {
    ...
    currentPrice: c.at(-1).c,
    inSupportZone: supportLevels.some(s => Math.abs(s.price - c.at(-1).c) / s.price < 0.02),
    inResistanceZone: resistanceLevels.some(r => Math.abs(r.price - c.at(-1).c) / r.price < 0.02)
  };
}
...
async function extractPipeline(symbol, tf = "1h") {
  try {
    const c = await candles(symbol, tf, tf.startsWith("m") ? 500 : 200);
...
async function macro2Y(symbol) {
  try {
    const c = await candles(symbol, "2Y", 200);
    if (!c || c.length < 50) {
      return { 
        regime: "NEUTRAL", 
        atrZ: 0, 
    ...
    const currentPrice = c.at(-1).c;
    const sma50 = mean(prices.slice(-50));
    const sma200 = mean(prices.slice(-200).slice(0, 150));
...
function htfGate(macro, ltfDirection) {
  ...
  return { passed: true, reason: "HTF/LTF alignment OK" };
}
...
async function AIDecisionEnhanced(symbol, tf, accountBalance = ACCOUNT_BALANCE) {
  ...
  try {
    ...
    // Cross-asset and macro analysis
    const btcDom = await BTCDominance();
    const riskMode = await RiskOnOff();
    const macro = await macro2Y(symbol);
    const correlation = await CorrelationFilter(symbol);
    const htfGateResult = htfGate(macro, candleDir.includes("BUY") ? "BUY" : "SELL");
    const sess = sessionBias();
    const atrConfirmResult = atrConfirm(entry, entry, atrVal, 
      candleDir.includes("BUY") ? "BUY" : "SELL", 0.15);

    // Alignment scoring (0-100)
    let alignmentScore = 0;
    const scores = [];

    // Macro alignment (20 points)
    ...
    // All gates passed - calculate quality score
    let qualityScore = 0;
    const qualityComponents = [];

    // ATR Z-Score contribution (max 25)
    ...
    // Price action contribution (max 15)
...
async function handleTelegramCommand(msg) {
  ...
  // Track last execution for rate limiting
  const now = Date.now();
  const lastExec = LAST_EXECUTION.get(userId) || 0;
  if (now - lastExec < 2000) { // 2 second cooldown
