  ...
  async makeRequest(endpoint, method = 'GET', params = {}, isFutures = false) {
    ...
    try {
      ...
      const config = {
        ...
        headers,
        timeout: 15000,
        validateStatus: (status) => status < 500 // Accept 400 errors for debugging
      };
...
async function fetchLivePrice(symbol) {
  ...
  // Alternative: Try CoinGecko as backup (no API key needed for basic price)
  try {
    ...
    console.warn(`CoinGecko price fetch failed for ${symbol}:`, error.message);
  }
...
async function fetchCandlesComprehensive(symbol, tf, limit = 200) {
  ...
  if (!data || data.length < 10) {
    ...
    // Try a different valid timeframe
  ...
  async fetchMultiExchangePrices(symbol) {
    try {
      ...
      try {
        ...
        if (response.data && response.data[coinId] && response.data[coinId].usd) {
          coingeckoPrice = response.data[coinId].usd;
        }
      } catch (error) {
        console.warn(`CoinGecko price fetch failed for ${symbol}:`, error.message);
      }
      ...
      if (coingeckoPrice && !isNaN(coingeckoPrice)) {
        prices.coingecko = {
...
async function BTCDominance() {
  try {
    // Get BTC and total crypto market cap from CoinGecko
...
╚══════════════════════════════════════════════════════════╝
    ...
    console.log("2. ✅ 100% real data from Bitget, CoinGecko, and Binance APIs");
    ...
    console.log("8. ✅ Fixed all API rate limiting issues");
  }
