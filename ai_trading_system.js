...
class RealMarketData {
  ...
  async getOHLCV(symbol, timeframe = '1h', limit = 200) {
    ...
    try {
      ...
      const data = await this.fetchFromBinance(symbol, timeframe, limit) ||
                   await this.fetchFromCoinGecko(symbol, timeframe, limit) ||
                   await this.fetchFromCoinPaprika(symbol, timeframe, limit) ||
                   await this.fetchFromYahoo(symbol, timeframe, limit);
  ...
  async fetchFromBinance(symbol, timeframe, limit) {
    try {
      ...
      if (!binanceSymbol.endsWith('USDT') && !binanceSymbol.endsWith('BTC') && !binanceSymbol.endsWith('ETH')) {
        binanceSymbol += 'USDT';
      }
  ...
  async fetchFromCoinGecko(symbol, timeframe, limit) {
    try {
      ...
      // Extract coin ID from symbol (simplified)
      let coinId = symbol.toLowerCase().replace('usdt', '').replace('/', '');
      ...
      if (response.json && response.json.prices) {
        ...
        return prices.map((price, index) => ({
          ...
          high: price[1] * 1.001, // Approximate since we only have prices
          low: price[1] * 0.999,
          close: price[1],
          volume: response.json.total_volumes ? response.json.total_volumes[index][1] || 0 : 0
        })).slice(-limit);
      }
    } catch (error) {
  ...
  async fetchFromCoinPaprika(symbol, timeframe, limit) {
    try {
      ...
      // Map timeframe to CoinPaprika interval
      let interval = '1h';
  ...
  async fetchFromYahoo(symbol, timeframe, limit) {
    try {
      ...
      const range = timeframe === '1h' ? '7d' : timeframe === '4h' ? '30d' : '1y';
  ...
  async getCurrentPrice(symbol) {
    ...
    try {
      ...
      try {
        ...
        logger.debug(`Binance price failed: ${binanceError.message}`);
      }
      ...
      if (!price) {
        try {
          ...
          if (response.json && response.json[coinId]) {
            price = response.json[coinId].usd;
          }
        } catch (coingeckoError) {
          logger.debug(`CoinGecko price failed: ${coingeckoError.message}`);
        }
      }
      
      // If still no price, try Yahoo
      if (!price) {
        try {
          let yahooSymbol = symbol.replace('/', '-');
          if (!yahooSymbol.includes('-')) {
          ...
          if (response.json && response.json.chart && response.json.chart.result) {
            const result = response.json.chart.result[0];
            const quotes = result.indicators.quote[0];
            price = quotes.close[quotes.close.length - 1] || null;
          }
        } catch (yahooError) {
          logger.debug(`Yahoo price failed: ${yahooError.message}`);
        }
      }
  ...
  async closePosition(positionId, exchange = 'binance') {
    ...
    try {
      if (MODE === 'live') {
        ...
        let binanceSymbol = position.symbol.replace('/', '').toUpperCase();
        if (!binanceSymbol.endsWith('USDT') && !binanceSymbol.endsWith('BTC') && !binanceSymbol.endsWith('ETH')) {
          binanceSymbol += 'USDT';
        }
...
class AI_Trading_System {
  ...
  async handleTest(req, res) {
    // Test market data
    const marketData = new RealMarketData();
    const btcPrice = await marketData.getCurrentPrice('BTCUSDT');
...
<body>
    <div class="container">
        ...
        <div class="live-data">
            ...
            <p>BTC Price: $${btcPrice ? btcPrice.toFixed(2) : 'Loading...'}</p>
