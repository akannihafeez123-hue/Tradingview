...
// Based on Bitget API documentation - these are the EXACT valid parameters
// FIXED: Using lowercase '1h' instead of '1H' based on error message
const BITGET_TF_MAP = {
  // Standard timeframes
  "1m": "1min",
  "3m": "3min", 
  ...
  "15m": "15min", 
  "30m": "30min", 
  "1h": "1h",  // FIXED: Changed from '1H' to '1h'
  "2h": "2h",  // FIXED: Changed from '2H' to '2h'
  "4h": "4h",  // FIXED: Changed from '4H' to '4h'
  "6h": "6h",  // FIXED: Changed from '6H' to '6h'
  "12h": "12h", // FIXED: Changed from '12H' to '12h'
  "1d": "1day",
  "3d": "3day",
  "1w": "1week",
  "1M": "1M",
...
// Valid Bitget timeframes from their API error message (as shown in logs)
const BITGET_VALID_TIMEFRAMES = [
  "1min", "3min", "5min", "15min", "30min", 
  "1h", "4h", "6h", "12h",  // FIXED: Using lowercase 'h'
  "1day", "1week", "1M", 
...
const BITGET_TF_REVERSE_MAP = {
  ...
  "15min": "15m",
  "30min": "30m", 
  "1h": "1h",  // FIXED: Using lowercase
  "2h": "2h",  // FIXED: Using lowercase
  "4h": "4h",  // FIXED: Using lowercase
  "6h": "6h",  // FIXED: Using lowercase
  "12h": "12h", // FIXED: Using lowercase
  "1day": "1d",
  "3day": "3d",
  "1week": "1w",
  "1M": "1M",
...
class BitgetAPI {
  ...
  async getKlines(symbol, timeframe = '1h', limit = 200) {  // FIXED: Default to lowercase
    const endpoint = `/api/v2/spot/market/candles`;
    
    // Convert timeframe to Bitget format - MUST be exact from valid list
    let bitgetTimeframe = BITGET_TF_MAP[timeframe];
    
    // If not in map, check if it's already valid
    if (!bitgetTimeframe || !BITGET_VALID_TIMEFRAMES.includes(bitgetTimeframe)) {
      // Try to find closest valid timeframe - FIXED: Use lowercase
  ...
  async getFuturesKlines(symbol, timeframe = '1h', limit = 200) {  // FIXED: Default to lowercase
...
async function fetchLivePrice(symbol) {
  ...
  // Alternative: Try Binance as backup (public API)
  try {
  ...
  // Alternative: Try Kraken as backup (public API)
  try {
...
async function fetchCandles(symbol, tf, limit = 200) {
  ...
  try {
    ...
    // Process candle data - Bitget returns: [timestamp, open, high, low, close, volume, quoteVolume]
    const candles = data.map(candle => {
      try {
        // Parse timestamp (might be in milliseconds or seconds)
...
async function fetchCandlesComprehensive(symbol, tf, limit = 200) {
  ...
  // Handle yearly timeframes by aggregating weekly data
  if (tf === "1y" || tf === "2y") {
    ...
    if (weekly && weekly.length >= 10) {
      // Aggregate weekly candles into yearly
      const yearlyCandles = [];
      const weeksPerYear = 52;
      
      for (let i = 0; i < weekly.length; i += weeksPerYear) {
        const yearChunk = weekly.slice(i, i + weeksPerYear);
        if (yearChunk.length < 4) continue; // Need at least 4 weeks for a meaningful year
  ...
  // For other timeframes, use the standard fetch
  ...
  if (!data || data.length < 10) {
    ...
    // Try a different valid timeframe - FIXED: Use lowercase
  ...
  async detectWhaleActivity(symbol) {
    try {
      ...
      if (orderBook && orderBook.code === '00000' && orderBook.data) {
        ...
        // Check for large orders (whale activity)
      ...
      if (recentTrades && recentTrades.code === '00000' && recentTrades.data) {
        ...
        // Analyze recent trades for whale patterns
...
class LiquidationRiskAnalyzer {
  ...
  detectLiquidationClusters(symbol) {
    // Implement actual cluster detection from real data
    return [];
  }
...
class CrossExchangeArbitrage {
  ...
  async fetchMultiExchangePrices(symbol) {
    try {
      ...
      // Get Binance price through public API
...
class NewsSentimentAnalyzer {
  constructor() {
    ...
    this.minFetchInterval = 60000; // 1 minute between fetches to avoid rate limiting
  }
  ...
  async analyzeMarketSentiment() {
    try {
      ...
      if (now - this.lastFetchTime < this.minFetchInterval) {
        // Return cached result if fetched recently
      ...
      // Calculate sentiment based on real price changes
      ...
      try {
        // Use CryptoCompare as backup for fear & greed
...
async function BTCDominance() {
  try {
    // Try multiple sources for BTC dominance
    let dominance = null;
    ...
    if (!dominance) {
      try {
        // Alternative method: Calculate from major exchanges
        const btcMarketCap = 950000000000; // Approximate BTC market cap
        const totalCryptoMarketCap = 2000000000000; // Approximate total crypto market cap
    ...
    return dominance ? round(dominance, 2) : 50; // Default to 50% if all sources fail
  } catch (e) { 
...
async function CorrelationFilter(symbol) {
  try {
    const btc = await fetchCandlesComprehensive("BTCUSDT", "1h", 100);  // FIXED: Use lowercase
    const s = await fetchCandlesComprehensive(symbol, "1h", 100);  // FIXED: Use lowercase
...
async function multiTimeframeConfirmation(symbol, direction) {
  const timeframes = ["15min", "1h", "4h"];  // FIXED: Use lowercase
  const results = [];
...
class EnhancedQuantumSignalGenerator {
  ...
  async generateEnhancedQuantumSignal(symbol, timeframe = "1h") {  // FIXED: Default to lowercase
    try {
      ...
      if (!candles || candles.length < 50) {
        ...
        // Try with smaller limit if first fetch failed
        ...
        // Use fallback candles
        console.log(`⚠️ Using fallback candles for ${symbol} ${timeframe}: ${fallbackCandles.length} candles`);
    ...
    try {
      switch(command) {
        ...
        case "/signal":
          ...
          const tf = args[2] || "1h";  // FIXED: Default to lowercase
    ...
    try {
      switch(command) {
        ...
        case "/analyze":
          ...
          const analyzeTf = args[2] || "1h";  // FIXED: Default to lowercase
          if (!analyzeSymbol) {
  ...
  async quantumScan() {
    ...
    for (const symbol of symbols) {
      ...
      try {
        ...
        const signal = await this.signalGenerator.generateEnhancedQuantumSignal(symbol, "1h");  // FIXED: Use lowercase
...
╚══════════════════════════════════════════════════════════╝
    ...
    // Check if Bitget API credentials are available
    ...
    this.pipelineInterval = setInterval(async () => {
      try {
        if (WATCH.size > 0) {
          ...
          for (const symbol of symbols) {
            await this.signalGenerator.generateEnhancedQuantumSignal(symbol, "1h");  // FIXED: Use lowercase
            await sleep(500);
          }
        }
      } catch (error) {
