...
/* =========================================================
   ...
   CRITICAL FIX: Dependency installation before imports
   This version handles module installation properly
========================================================= */

/* ================= DEPENDENCY AUTO-INSTALLER - ULTRA FIXED ================= */
...
// Function to check and install dependencies BEFORE importing
function installDependenciesSync() {
  ...
  try {
    ...
    console.log(`📦 Installing: ${Object.keys(DEPENDENCIES).join(', ')}`);
    
    // Use execSync with proper error handling
    try {
      execSync(`npm install ${depsToInstall} --no-save --no-package-lock --loglevel=error`, {
        stdio: 'inherit',
        cwd: __dirname,
        encoding: 'utf8'
      });
      ...
      console.warn('⚠️ Bulk installation failed, trying individual packages...');
      
      // Install packages one by one
      for (const [dep, version] of Object.entries(DEPENDENCIES)) {
        try {
          console.log(`Installing ${dep}@${version}...`);
          execSync(`npm install ${dep}@${version} --no-save --loglevel=error`, {
            stdio: 'pipe',
            cwd: __dirname,
            encoding: 'utf8'
          });
          ...
          // Try without version
          try {
            execSync(`npm install ${dep} --no-save --loglevel=error`, {
              stdio: 'pipe',
              cwd: __dirname,
              encoding: 'utf8'
            });
            console.log(`✅ ${dep} installed (latest)`);
          } catch (e2) {
            console.error(`Completely failed to install ${dep}:`, e2.message);
            throw new Error(`Failed to install required dependency: ${dep}`);
          }
        }
      }
    }
    ...
    for (const dep of Object.keys(DEPENDENCIES)) {
      try {
        ...
        console.error(`❌ ${dep} not found after installation!`);
        throw new Error(`Dependency ${dep} installation failed`);
      }
    }
    ...
    console.error('npm install ws axios moment lodash mathjs dotenv winston node-fetch');
    process.exit(1);
  }
}

// Install dependencies BEFORE any imports
