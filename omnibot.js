...
const normalizeSymbol = s => {
  if (!s) return "";
  // Convert to uppercase and remove any slashes, dashes, or spaces
  let normalized = s.toUpperCase().replace(/[\/\-\s]/g, "");
  ...
  if (symbolMappings[s.toUpperCase()]) {
    normalized = symbolMappings[s.toUpperCase()];
  }
  
  // For crypto symbols, ensure they end with USDT if not already
...
async function candles(symbol, tf, limit = 1000) {
  try {
    ...
    if (!s || s.length < 3) {
      console.warn(`Invalid symbol format: ${symbol} -> ${s}`);
      return [];
    }
    
    console.log(`🔍 Fetching candles for ${symbol} (normalized: ${s}) ${tf}...`);
    
    // Try multiple endpoints for robustness
    const endpoints = [
      `https://api.binance.com/api/v3/klines?symbol=${s}&interval=${tf}&limit=${limit}`,
      `https://api3.binance.com/api/v3/klines?symbol=${s}&interval=${tf}&limit=${limit}`,
      `https://api1.binance.com/api/v3/klines?symbol=${s}&interval=${tf}&limit=${limit}`,
      `https://api.binance.us/api/v3/klines?symbol=${s}&interval=${tf}&limit=${limit}`,
      `https://api.binance.me/api/v3/klines?symbol=${s}&interval=${tf}&limit=${limit}`
    ];
    ...
    for (const url of endpoints) {
      try {
        ...
        if (r && Array.isArray(r) && r.length > 0) {
          ...
          console.log(`✅ Fetched ${candles.length} candles for ${symbol} (${s}) ${tf} from ${url.split('/')[2]}`);
          return candles;
        } else if (r && r.code) {
          console.warn(`API error from ${url.split('/')[2]}: ${r.msg || 'Unknown error'}`);
          lastError = r.msg;
        } else if (r && Array.isArray(r) && r.length === 0) {
          console.warn(`Empty data from ${url.split('/')[2]}`);
        }
      } catch (error) {
        lastError = error.message;
        console.warn(`Failed to fetch from ${url.split('/')[2]}: ${error.message}`);
        await sleep(500); // Wait before trying next endpoint
      }
    }
    
    console.warn(`❌ No data for ${symbol} (${s}) ${tf} from any endpoint${lastError ? `: ${lastError}` : ''}`);
    
    // Fallback: Try CoinGecko for BTC if Binance fails
    if (s.includes("BTC") && tf === "1d") {
      console.log(`🔄 Trying CoinGecko fallback for ${s}...`);
      try {
        const cgData = await fetchJSON(`https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=${limit/24}`);
        if (cgData && cgData.prices) {
          const cgCandles = cgData.prices.slice(-limit).map((price, index, arr) => {
            if (index === 0) return null;
            return {
              t: price[0],
              o: arr[index-1][1],
              h: Math.max(arr[index-1][1], price[1]),
              l: Math.min(arr[index-1][1], price[1]),
              c: price[1],
              v: 0,
              bullish: price[1] > arr[index-1][1]
            };
          }).filter(x => x !== null);
          
          if (cgCandles.length > 0) {
            console.log(`✅ Fallback: Got ${cgCandles.length} candles from CoinGecko`);
            return cgCandles;
          }
        }
      } catch (cgError) {
        console.warn(`CoinGecko fallback also failed: ${cgError.message}`);
      }
    }
...
<b>💡 SYMBOL EXAMPLES:</b>
• <code>/analyze BTC/USDT 1h</code>
• <code>/analyze ETH/USDT 4h</code>
• <code>/analyze BTCUSDT 1h</code> (no slash)
• <code>/swing SOL/USDT</code>
• <code>/scalp XRP/USDT</code>

<i>Supported: BTC, ETH, BNB, SOL, XRP, ADA, DOT, DOGE, etc.</i>
    `.trim();
  ...
  if (cmd === "/swing" || cmd === "/scalp") {
    ...
    if (!symbol) {
      return send(chatId, 
        ...
        `Example: <code>${cmd} BTC/USDT</code>\n` +
  ...
  if (cmd === "/watch") {
    ...
    if (!symbol) {
      return send(chatId, 
        ...
        "Example: <code>/watch BTC/USDT</code>\n" +
  ...
  if (cmd === "/unwatch") {
    ...
    if (!symbol) {
      return send(chatId, 
        ...
        "Example: <code>/unwatch BTC/USDT</code>"
      );
    }
...
╚══════════════════════════════════════════════════════════╝
  ...
  try {
    const testData = await candles("BTC/USDT", "1h", 10);
    if (testData.length > 0) {
      console.log(`✅ Data connection working: Fetched ${testData.length} BTC/USDT candles`);
    } else {
      console.warn("⚠️ Could not fetch test data. Trying alternative...");
      const testData2 = await candles("BTCUSDT", "1h", 10);
      if (testData2.length > 0) {
        console.log(`✅ Data connection working: Fetched ${testData2.length} BTCUSDT candles`);
      } else {
...
<i>Session: ${decision.session} | ATR: ${decision.atrPercent}%</i>
  ...
  console.log("📈 Symbol formats accepted: BTC/USDT, BTCUSDT, ETH/USDT, etc.");
