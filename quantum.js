...
/* ================= NETWORK FUNCTIONS WITH RATE LIMITING ================= */
const quantumFetch = async (url, options = {}) => {
  try {
    ...
    if (error.response && error.response.status === 429) {
      console.warn(`Rate limited for ${url}, waiting 2 seconds...`);
      await sleep(2000);
    }
...
async function fetchLivePrice(symbol) {
  ...
  try {
    ...
    const response = await quantumFetch(`https://api.binance.com/api/v3/ticker/price?symbol=${symbol}`);
    ...
    console.warn(`Binance fallback failed for ${symbol}:`, error.message);
  }
  
  // Fallback 2: Bybit API
  try {
    const coinId = symbol.toLowerCase().replace('usdt', '');
    const response = await quantumFetch(`https://api.bybit.com/v5/market/tickers?category=spot&symbol=${symbol}`);
    if (response && response.result && response.result.list && response.result.list[0]) {
      ...
      if (!isNaN(price)) {
        updateTick(symbol, price);
        console.log(`✅ Bybit fallback price for ${symbol}: ${price}`);
        return price;
      }
    }
  } catch (error) {
    console.warn(`Bybit fallback failed for ${symbol}:`, error.message);
  }
  
  // Last resort: check cache
  const cached = TICK_STATE.get(symbol);
  if (cached && cached.last && !isNaN(cached.last)) {
    console.log(`⚠️ Using cached price for ${symbol}: ${cached.last}`);
    return cached.last;
  }
...
async function fetchCandles(symbol, tf, limit = 200) {
  ...
  try {
    ...
    try {
      const interval = tf === '1H' ? '1h' : tf.toLowerCase();
      const response = await quantumFetch(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=${limit}`);
      if (response && Array.isArray(response)) {
        const candles = response.map(candle => ({
          ...
          qv: parseFloat(candle[7]),
          bullish: parseFloat(candle[4]) > parseFloat(candle[1])
        })).filter(c => !isNaN(c.c) && !isNaN(c.o));
        
        if (candles.length >= 10) {
          console.log(`✅ Binance fallback candles for ${symbol} ${tf}: ${candles.length}`);
          return candles.reverse();
        }
      }
    } catch (binanceError) {
      console.warn(`Binance fallback failed for ${symbol} ${tf}:`, binanceError.message);
    }
...
function generateSyntheticCandles(candles, timeframe) {
  ...
  const groupSize = candlesPerGroup[timeframe] || 1;
...
class CrossExchangeArbitrage {
  ...
  async fetchMultiExchangePrices(symbol) {
    try {
      ...
      // Try Binance as comparison
      let binancePrice = null;
      try {
        const response = await quantumFetch(`https://api.binance.com/api/v3/ticker/price?symbol=${symbol}`);
        if (response && response.price) {
          binancePrice = Number(response.price);
        }
      } catch (error) {
        console.warn(`Binance price fetch failed for ${symbol}:`, error.message);
      }
      
      // Try Bybit as comparison
      let bybitPrice = null;
      try {
        const response = await quantumFetch(`https://api.bybit.com/v5/market/tickers?category=spot&symbol=${symbol}`);
        if (response && response.result && response.result.list && response.result.list[0]) {
          bybitPrice = Number(response.result.list[0].lastPrice);
        }
      } catch (error) {
        console.warn(`Bybit price fetch failed for ${symbol}:`, error.message);
      }
...
async function BTCDominance() {
  try {
    // Use alternative API since CoinGecko has rate limits
    const response = await quantumFetch("https://api.binance.com/api/v3/ticker/24hr");
    if (response && Array.isArray(response)) {
      // Calculate approximate dominance from top coins
      const btc = response.find(t => t.symbol === "BTCUSDT");
      const eth = response.find(t => t.symbol === "ETHUSDT");
      const bnb = response.find(t => t.symbol === "BNBUSDT");
      
      if (btc && eth && bnb) {
        const btcVol = parseFloat(btc.volume);
        const ethVol = parseFloat(eth.volume);
        const bnbVol = parseFloat(bnb.volume);
        const totalVol = btcVol + ethVol + bnbVol;
        
        if (totalVol > 0) {
          const dominance = (btcVol / totalVol) * 100;
...
╚══════════════════════════════════════════════════════════╝
    ...
    console.log("1. ✅ Correct Bitget API endpoints implemented");
    console.log("2. ✅ Multiple fallback exchanges (Binance, Bybit)");
    console.log("3. ✅ Enhanced error handling and rate limiting");
    console.log("4. ✅ Proper symbol formatting and validation");
    ...
    console.log("8. ✅ Multi-exchange arbitrage detection");
  }
