...
class BitgetAPI {
  ...
  async getKlines(symbol, timeframe = '1h', limit = 200) {
    ...
    const params = {
      symbol,
      granularity: BITGET_TF_MAP[timeframe] || timeframe,
      limit
    };
    return await this.makeRequest(endpoint, 'GET', params);
  }
...
async function fetchLivePrice(symbol) {
  ...
  try {
    const response = await quantumFetch(`https://api.coingecko.com/api/v3/simple/price?ids=${symbol.toLowerCase().replace('usdt', '')}&vs_currencies=usd`);
    if (response && response[symbol.toLowerCase().replace('usdt', '')]) {
      return response[symbol.toLowerCase().replace('usdt', '')].usd;
    }
  } catch (error) {
...
async function fetchCandles(symbol, tf, limit = 200) {
  try {
    ...
    if (!data || !data.data || !Array.isArray(data.data) || data.data.length < 20) return null;
    ...
    return data.data.map(candle => ({
      ...
      qv: parseFloat(candle[6]) || parseFloat(candle[5]) * (parseFloat(candle[4]) + parseFloat(candle[1])) / 2,
      bullish: parseFloat(candle[4]) > parseFloat(candle[1])
    })).reverse(); // Bitget returns newest first, reverse to get chronological order
  } catch (error) {
    console.warn(`Bitget candles fetch failed for ${symbol}:`, error.message);
    return null;
  }
}
...
async function fetchQuantumPrice(symbol) {
  ...
  const endpoints = [
    async () => {
      const data = MARKET_TYPE === "futures" 
        ? await bitgetAPI.getFuturesTicker(cleanSymbol)
        : await bitgetAPI.getTicker(cleanSymbol);
      if (data && data.data) {
        if (MARKET_TYPE === "futures" && data.data[0]) {
    ...
    async () => {
      try {
        const response = await quantumFetch(`https://api.binance.com/api/v3/ticker/price?symbol=${cleanSymbol}`);
...
class BitgetWebSocketManager {
  ...
  processTradeData(symbol, data) {
    ...
    const metrics = this.calculateTradeFlowMetrics(this.tradeFlows.get(symbol));
...
class OptionsFlowAnalyzer {
  ...
  calculateMaxPain(optionsData) {
    ...
    const strikes = [...new Set(optionsData.map(o => o.strike).filter(s => s))].sort((a, b) => a - b);
...
class LiquidationRiskAnalyzer {
  ...
  async fetchLiquidationData(symbol) {
    try {
      if (MARKET_TYPE === "futures") {
        const data = await bitgetAPI.getLiquidationOrders(symbol);
        if (!data || !data.data) {
          return this.simulateLiquidationData(symbol);
        }
        
        return this.parseLiquidationData(data.data, symbol);
      }
  ...
  parseLiquidationData(liquidations, symbol) {
    if (!Array.isArray(liquidations)) return this.simulateLiquidationData(symbol);
    
    const longLiquidations = liquidations.filter(l => l.side === 'long');
    const shortLiquidations = liquidations.filter(l => l.side === 'short');
    
    const longLiqVolume = longLiquidations.reduce((sum, l) => sum + parseFloat(l.size || 0), 0);
    const shortLiqVolume = shortLiquidations.reduce((sum, l) => sum + parseFloat(l.size || 0), 0);
  ...
  findLiquidationClusters(liquidations) {
    ...
    const clusters = [];
    liquidations.sort((a, b) => (a.ts || 0) - (b.ts || 0));
    ...
    for (const liq of liquidations) {
      if (currentCluster.length === 0) {
        ...
        if ((liq.ts || 0) - (lastLiq.ts || 0) <= TIME_WINDOW) {
          currentCluster.push(liq);
        } else {
          if (currentCluster.length >= 3) {
            clusters.push({
              start: currentCluster[0].ts || 0,
              end: currentCluster[currentCluster.length - 1].ts || 0,
              count: currentCluster.length,
              totalVolume: currentCluster.reduce((sum, l) => sum + parseFloat(l.size || 0), 0),
              avgPrice: mean(currentCluster.map(l => parseFloat(l.price || 0)))
            });
          }
          currentCluster = [liq];
        }
  ...
  estimateLiquidationLevels(symbol, liquidations) {
    ...
    const priceLevels = liquidations.map(l => parseFloat(l.price || 0)).filter(p => p > 0);
...
class CrossExchangeArbitrage {
  ...
  async fetchMultiExchangePrices(symbol) {
    const exchanges = [
      { name: 'bitget', spot: async () => {
        const data = await bitgetAPI.getTicker(symbol);
        return data?.data?.last ? parseFloat(data.data.last) : null;
      }},
      { name: 'bitget_futures', futures: async () => {
        const data = await bitgetAPI.getFuturesTicker(symbol);
        return data?.data?.[0]?.last ? parseFloat(data.data[0].last) : null;
      }},
      { name: 'binance', spot: async () => {
        try {
          const response = await quantumFetch(`https://api.binance.com/api/v3/ticker/price?symbol=${symbol}`);
          return response?.price ? parseFloat(response.price) : null;
        } catch { return null; }
      }},
      { name: 'bybit', spot: async () => {
        try {
          const response = await quantumFetch(`https://api.bybit.com/v2/public/tickers?symbol=${symbol}`);
          return response?.result?.[0]?.last_price ? parseFloat(response.result[0].last_price) : null;
        } catch { return null; }
      }}
    ];
    ...
    for (const exchange of exchanges) {
      try {
        ...
        if (exchange.name === 'bitget_futures') {
          price = await exchange.futures();
        } else {
          price = await exchange.spot();
        }
  ...
  async fetchFundingRates(symbol) {
    ...
    try {
      const data = await bitgetAPI.getFundingRate(symbol);
      if (data && data.data) {
        rates.bitget = {
          rate: parseFloat(data.data[0]?.fundingRate || 0),
          nextFunding: data.data[0]?.fundingTime
        };
      }
    } catch (error) {
      console.warn('Bitget funding rate fetch failed:', error.message);
    }
...
class EnhancedQuantumSignalGenerator {
  ...
  agentPriceAction(candles) {
    ...
    const sma5 = mean(candles.slice(-10, -5).map(c => c.c));
...
Timestamp: ${new Date(orderBook.timestamp).toISOString()}
  ...
  async quantumScan() {
    console.log('Starting quantum scan...');
    const signals = [];
