...
// Check for required dependencies without importing them first
const fs = require('fs');
...
// Check each dependency by trying to resolve it
for (const [dep, name] of Object.entries(requiredDependencies)) {
  try {
    require.resolve(dep);
    console.log(`✅ ${name} loaded`);
  } catch (e) {
    console.log(`❌ ${name} not installed. Please run: npm install ${dep}`);
    hasAllDeps = false;
