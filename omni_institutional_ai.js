#!/usr/bin/env node
/* =========================================================
   OMNI INSTITUTIONAL AI — FULLY READY-TO-RUN (Macro + Scalp + Cross-Asset)
...
/* ================= CANDLE PATTERN ================= */
function candlePattern(c) { if (c.length < 3) return "None"; const last = c.slice(-3), [c1,c2,c3] = last; if(c2.c>c2.o && c2.o<c1.c && c2.c>c1.o && c3.c>c3.o) return "BullishEngulfing"; if(c2.c<c2.o && c2.o>c1.c && c2.c<c1.o && c3.c<c3.o) return "BearishEngulfing"; if(c3.c>c3.o && (c3.c-c3.o)<(c3.h-c3.l)*0.3 && (c3.o-c3.l)>(c3.h-c3.c)) return "Hammer"; return "Neutral"; }
...
async function AIDecision(symbol, tf, macro, btcDom){
  ...
  for(let i=0;i<AI_COUNT;i++){
    ...
    let qualityScore = 30*Math.min(1,volZ/2)+30*Math.min(1,Math.abs(delta)/mean(c.map(x=>x.v)))+30*Math.min(1,relVol/2)+(bos?5:0)+(absorption?5:0);
    qualityScore *= 0.7+0.3*(pipelineDatabase.history.filter(h=>h.symbol===symbol).length/50);
    if(macro){ if(macro.regime==="Range") qualityScore*=0.7; if(macro.regime==="Neutral") qualityScore*=0.85; }
    ...
    if(typeof btcDom==="number"){ if(btcDom>60) direction="SELL"; if(btcDom<40) direction="BUY"; }
...
async function handleTelegramCommand(msg){
  ...
  if(cmd==="/swing" || cmd==="/scalp"){
    ...
    for(const tf of tfs){
      ...
      results.push(`${tf} | ${decision.direction} | ${decision.quality.toFixed(2)}% | Entry: ${decision.entry.toFixed(2)} | ATR: ${decision.
