...
/* =========================================================
   ...
   Full Institutional Strategies | Pure JS ML | Self-Learning | Telegram Alerts
========================================================= */
...
const ALERT_THRESHOLD = parseFloat(process.env.ALERT_THRESHOLD) || 0.6;
const WATCH_ALERT_THRESHOLD = parseFloat(process.env.WATCH_ALERT_THRESHOLD) || 0.8;
const WATCH_INTERVAL_MS = parseInt(process.env.WATCH_INTERVAL_MS) || 30000;
const ML_LEARNING_RATE = parseFloat(process.env.ML_LEARNING_RATE) || 0.01;
...
/* ================= PURE JS ML ENGINE ================= */
class SimpleML {
  constructor(inputSize, hiddenSize, outputSize, lr=ML_LEARNING_RATE){
  ...
  forward(input){
    this.input=input;
    this.h = this.relu(this.W1.map((row,i)=>row.reduce((sum,v,j)=>sum+v*input[j],0)+this.b1[i]));
    this.output = this.softmax(this.W2.map((row,i)=>row.reduce((sum,v,j)=>sum+v*this.h[j],0)+this.b2[i]));
    return this.output;
  }
  train(input, target){
...
function analyzeTF(c){
  const volZ = ATR_Z(c), atr = ATR(c), delta = VolumeDelta(c), relVol = RelativeVolume(c);
  const bos = BOS(c), choch = CHoCH(c), liq = LiquidityVoid(c), stopHunt = StopHuntProb(c), absorption = Absorption(c);
...
function TradeSignals(c,f,newsImpact=0){
  const entry = c.at(-1).c, atr = f.atr || 0;
  let dir = f.direction;
  if(newsImpact>0.1) dir="BUY"; else if(newsImpact<-0.1) dir="SELL";
  const sl = dir==="BUY"?entry-atr*1:entry+atr*1;
...
async function analyze(symbol, chat){
  const results=[]; let marketUsed=null;
  let voteScore={BUY:0,SELL:0}, stratVotes={}, totalQuality=0;
  const newsImpact = await newsScore(symbol);
  for(const tf of topDownTFs){
    const r = await getMarketCandles(symbol,tf); if(!r.candles.length) continue;
    marketUsed=r.market;
    const f = analyzeTF(r.candles);
    const s = TradeSignals(r.candles,f,newsImpact);
    const strat = ["Liquidity","Breakout","Momentum","MeanRev","Quantum"][Math.floor(Math.random()*5)];
  ...
  if(avgQuality<ALERT_THRESHOLD){ watchlist.set(symbol,{chat,lastAnalysis:Date.now()}); send(chat,`Market for ${symbol} low confidence — added to watchlist`); return; }
  ...
  while(true){
    for(const [symbol,data] of watchlist){
      try{
        ...
        const f = analyzeTF(r.candles);
        if(f.qualityScore>=WATCH_ALERT_THRESHOLD){ await analyze(symbol,data.chat); watchlist.delete(symbol); }
      }catch{}
    }
  ...
  while(true){
    ...
    for(const u of r.result){
      ...
      if(msg.startsWith("/analyze")){
        ...
        if(!s) continue;
        if(tfs){ for(const tf of tfs) await analyze(s,chat); } else await analyze(s,chat);
      }
    }
    await sleep(2000);
  }
