...
const AI_COUNT = 3;
const THRESHOLD_WATCH = 60;   // actual percentage now (0–100)
const THRESHOLD_EXECUTE = 80; // actual percentage now (0–100)
...
async function analyze(symbol,chat=TELEGRAM_CHAT_ID,mode="swing",tfs=null){
  ...
  if(confidence < THRESHOLD_WATCH){
    send(chat, `OMNI AI SIGNAL SYMBOL: ${symbol} MODE: ${mode} STATUS: WAIT | Confidence ${confidence.toFixed(2)}%`);
    return;
  }
