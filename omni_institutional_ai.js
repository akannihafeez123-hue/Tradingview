#!/usr/bin/env node
/* =========================================================
   OMNI INSTITUTIONAL AI — FULLY READY-TO-RUN
...
async function extractPipeline(symbol, tf="1h"){
  ...
  const strategy = {
    symbol, tf, liq, absorption, atrZ, deltaVol, sr, pattern,
...
async function AIDecision(symbol,tf){
  ...
  for(let i=0;i<AI_COUNT;i++){
    ...
    const qualityScore = Math.min(100, volZ*Math.abs(delta)*relVol*(bos?1.2:1)*(absorption?1.1:1));
...
async function analyze(symbol,chat=TELEGRAM_CHAT_ID,mode="swing",tfs=null){
  ...
  // Fetch macro (2Y top-down)
  const macroC = await candles(symbol,"2Y",1000);
  const macroTrend = macroC.length>0?(macroC.at(-1).c>macroC.at(0).c?"Bullish":"Bearish"):"Neutral";
  ...
  if(confidence < THRESHOLD_WATCH){
    send(chat, `OMNI AI SIGNAL SYMBOL: ${symbol} MODE: ${mode} STATUS: WAIT | Confidence ${confidence.toFixed(2)}% | MACRO: ${macroTrend} BTC Dom:${btcDom.toFixed(2)}%`);
    return;
  }
