...
function generateSyntheticCandles(candles, timeframe) {
  ...
  if (baseTimeframe === 'd') {
    // For 2d, we need to combine two 1d candles
...
async function fetchFromCoinGecko(symbol, days = 30) {
  try {
    ...
    const coinId = symbolMap[symbol] || symbol.toLowerCase().replace('usdt', '');
    const url = `https://api.coingecko.com/api/v3/coins/${coinId}/market_chart?vs_currency=usd&days=${days}&interval=daily`;
    
    const r = await fetchJSON(url);
    if (!r || !r.prices) return null;
    
    return r.prices.map(([timestamp, price], index) => ({
      t: timestamp,
      o: price,
      h: price * 1.02, // Approximate high
      l: price * 0.98, // Approximate low
      c: price,
      v: 0,
...
async function candles(symbol, tf, limit = 1000) {
  try {
    // Try Binance first
    let candles = await fetchFromBinance(symbol, tf, limit);
    
    // If Binance fails, try CoinGecko for daily data
    if (!candles || candles.length === 0) {
      console.log(`Trying CoinGecko for ${symbol} ${tf}...`);
      
      const days = tf.includes('d') ? parseInt(tf) * limit : 
                  tf.includes('w') ? 7 * limit :
                  tf.includes('M') ? 30 * limit : limit;
      
      candles = await fetchFromCoinGecko(symbol, Math.min(days, 365));
      
      if (candles) {
        // Generate synthetic candles for other timeframes
        if (tf === '2d') {
          candles = generateSyntheticCandles(candles, '2d');
        } else if (tf === '1w') {
          candles = generateSyntheticCandles(candles, '7d');
        } else if (tf === '1M') {
          candles = generateSyntheticCandles(candles, '30d');
        }
      }
    }
    ...
    if (!candles || candles.length === 0) {
      console.warn(`No data for ${symbol} ${tf}. Generating synthetic data...`);
      return generateSyntheticData(symbol, tf, limit);
    }
    
    return candles.slice(-limit); // Return only requested limit
  } catch (e) { 
...
function generateSyntheticData(symbol, tf, limit = 1000) {
...
async function AIDecisionEnhanced(symbol, tf, accountBalance = ACCOUNT_BALANCE) {
  ...
  try {
    ...
    if (!c.length || c.length < 30) {
      ...
      if (c.length > 0) {
        // Use whatever data we have, but warn
        console.log(`Using reduced dataset: ${c.length} candles`);
      } else {
        return { 
          success: false,
          direction: null, 
...
<i>Analysis time: ${decision.processingTime}ms</i>
  ...
  if (cmd === "/swing" || cmd === "/scalp") {
    ...
    for (const tf of tfs) {
      ...
      await sleep(1000); // Increased delay between timeframe analysis
    }
...
╚══════════════════════════════════════════════════════════╝
  ...
  try {
    ...
    console.log("⚠️ Data fetching test failed, using synthetic data");
  }
