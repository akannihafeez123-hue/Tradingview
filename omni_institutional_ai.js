...
/* =========================================================
   ...
   🔴 ADDED — Price Tracking | Session Bias | HTF Gates | ATR Confirmation
========================================================= */
...
async function AIDecision(symbol, tf, macro, btcDom) {
  ...
  if (SCALP_TF.includes(tf)) {
    if (!htfGate(macro, candleDir)) return {
      direction: null,
      quality: 0,
      strategies: [],
      entry,
  ...
  if (!atrConfirm(entry, entry + (tickDir === "BUY" ? 1 : -1), ATR(c), candleDir)) return {
    direction: null,
    quality: 0,
    strategies: [],
    entry,
    atr: ATR(c),
    gate: "ATR_INSUFFICIENT"
  };

  // 🔴 ADDED — Session bias
  const sess = sessionBias();

  // Original AI decision logic
  ...
  return {
    ...
    entry: c.at(-1).c,
    atr: ATR(c),
    session: sess.name, // 🔴 ADDED
    tickBias: tickDir, // 🔴 ADDED
    gate: "PASSED" // 🔴 ADDED
  };
}
  ...
  if (cmd === "/swing" || cmd === "/scalp") {
    ...
    for (const tf of tfs) {
      ...
      if (decision.direction) {
        results.push(`${tf} | ${decision.direction} | ${decision.quality.toFixed(2)}% | Entry: ${decision.entry.toFixed(2)} | ATR: ${decision.atr.toFixed(2)}`);
      }
    }
...
async function main() {
  ...
  // Start Telegram polling (you'll need to implement this)
  // Start pipeline extraction loop
