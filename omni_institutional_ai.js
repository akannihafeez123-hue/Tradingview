#!/usr/bin/env node
/* =========================================================
   OMNI INSTITUTIONAL AI — FULLY READY-TO-RUN (Macro Fix)
   ...
   NOTE: Macro (2Y) fallback and BTC dominance path fixed
========================================================= */
...
const AI_COUNT = 3;
const THRESHOLD_WATCH = 60; // actual percentage 0–100
const THRESHOLD_EXECUTE = 80; // actual percentage 0–100
const EXECUTION_COOLDOWN_MS = 60 * 1000; // per-asset cooldown after execution (example)
...
const BTCDominance = async () => {
  try {
    ...
    // coinGecko returns top-level market_cap_percentage or under "data"
...
/* ================= AI DECISION (macro + BTC bias supported) ================= */
async function AIDecision(symbol, tf, macro, btcDom) {
  ...
  for (let i = 0; i < AI_COUNT; i++) {
    ...
    qualityScore = Math.min(100, isFinite(qualityScore) ? qualityScore : 0);
...
/* ================= ANALYSIS (uses macro2Y robustly) ================= */
async function analyze(symbol, chat = TELEGRAM_CHAT_ID, mode = "swing", tfs = null) {
  ...
  // Get macro + BTC dominance (with safe fallbacks)
  const macro = await macro2Y(symbol);
  const macroRegime = macro?.regime ?? "Neutral";
  const btcDom = await BTCDominance();
  ...
  if (!count) {
    await send(chat, `${symbol} has insufficient data.`);
    return;
  }
  ...
  if (Date.now() - lastExec < EXECUTION_COOLDOWN_MS) {
    await send(chat, `${symbol} cooldown active. Skipping execution.`);
    return;
  }

  if (confidence < THRESHOLD_WATCH) {
    await send(chat, `OMNI AI SIGNAL SYMBOL: ${symbol} MODE: ${mode} STATUS: WAIT | Confidence ${confidence.toFixed(2)}% | MACRO: ${macroRegime} BTC Dom:${btcDom.toFixed(2)}%`);
    return;
  }

  // Mark last execution time (pre-send)
  ...
  if (mode === "swing") {
    ...
    // normalize extremes: prevent absurd ranges caused by macro ATR
    tp = Math.min(tp, entryPrice * 2);
    sl = Math.max(sl, entryPrice / 2);
  } else {
  ...
  while (true) {
    ...
    if (r?.result) for (const u of r.result) {
      ...
      if (m.startsWith("/swing")) {
        const [_, sym, tfsStr] = m.split(" ");
        const tfs = tfsStr?.split(",");
        analyze(sym, chat, "swing", tfs);
      } else if (m.startsWith("/scalp")) {
        const [_, sym, tfsStr] = m.split(" ");
        const tfs = tfsStr?.split(",");
        analyze(sym, chat, "scalp", tfs);
      } else if (m.startsWith("/analyze")) {
        const [_, sym, modeStr, tfsStr] = m.split(" ");
        const tfs = tfsStr?.split(",");
        analyze(sym, chat, modeStr || "swing", tfs);
      } else if (m.startsWith("/watch")) {
        const [_, sym, md] = m.split(" ");
        if (sym) WATCH.set(sym.toUpperCase(), { chat, mode: md || "swing", tfs: null });
        send(chat, `Watching ${sym}`);
      } else if (m.startsWith("/unwatch")) {
        const [_, sym] = m.split(" ");
        if (sym) { WATCH.delete(sym.toUpperCase()); send(chat, `Stopped watching ${sym}`); }
      }
    }
    await sleep(5000);
  }
