...
/* ================= DEPENDENCY AUTO-INSTALLER - FIXED VERSION ================= */
...
function installDependenciesSync() {
  ...
  if (missingDeps.length > 0) {
    ...
    try {
      // Create a temporary package.json with required dependencies
      const packageJsonPath = join(__dirname, 'package-temp.json');
      const packageJson = {
      ...
      // Install ALL dependencies at once (more reliable)
      console.log('Installing: ' + Object.keys(DEPENDENCIES).join(', '));
      
      execSync(`npm install ${Object.keys(DEPENDENCIES).map(dep => `${dep}@${DEPENDENCIES[dep]}`).join(' ')} --no-save --no-package-lock --loglevel=error`, {
        stdio: 'inherit',
        cwd: __dirname
      });
    ...
    // Give Node.js a moment to recognize the new modules
