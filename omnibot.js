...
const normalizeSymbol = s => {
  if (!s) return "";
  // Remove any non-alphanumeric characters and ensure proper format
  return s.toUpperCase().replace(/[^A-Z0-9]/g, "");
};

/* ================= SYNTHETIC TIMEFRAME ENGINE ================= */
function synthesize(baseCandles, factor) {
  const out = [];
  for (let i = 0; i + factor <= baseCandles.length; i += factor) {
    const s = baseCandles.slice(i, i + factor);
    out.push({
      t: s[0].t,
      o: s[0].o,
...
/* ================= TIME FRAME RESOLVER ================= */
async function getCandlesWithSynthetic(symbol, tf) {
  if (tf === "2d" || tf === "3d") {
    const base = await fetchCandles(symbol, "1d", 400);
    return tf === "2d" ? synthesize(base, 2) : synthesize(base, 3);
  }
  return await fetchCandles(symbol, tf);
}

/* ================= ENHANCED CANDLE FETCHER WITH SYNTHETIC SUPPORT ================= */
async function fetchCandles(symbol, interval, limit = 1000) {
  try {
    ...
    const endpoints = [
      ...
      `https://api3.binance.com/api/v3/klines?symbol=${s}&interval=${interval}&limit=${limit}`,
      `https://api1.binance.com/api/v3/klines?symbol=${s}&interval=${interval}&limit=${limit}`
    ];
    ...
    for (const url of endpoints) {
      try {
        ...
        await sleep(500); // Wait before trying next endpoint
      }
    }
    
    console.warn(`No data for ${symbol} ${interval} from any endpoint${lastError ? `: ${lastError.message}` : ''}`);
    return [];
    
  } catch (e) { 
    console.error(`Candle fetch error for ${symbol} ${interval}:`, e.message);
    return []; 
  }
}

/* ================= UPDATED CANDLES FUNCTION WITH SYNTHETIC SUPPORT ================= */
async function candles(symbol, tf, limit = 1000) {
  return await getCandlesWithSynthetic(symbol, tf, limit);
}
...
async function extractPipeline(symbol, tf = "1h") {
  try {
    ...
    if (c.length < 50) {
      console.warn(`Insufficient data for pipeline: ${symbol} ${tf} (${c.length} candles)`);
      return;
    }
...
async function macro2Y(symbol) {
  try {
    ...
    if (!c || c.length < 50) {
      console.warn(`Insufficient data for macro analysis: ${symbol} (${c?.length || 0} candles)`);
      return { 
        regime: "NEUTRAL", 
        atrZ: 0, 
        trend: "SIDEWAYS", 
...
async function AIDecisionEnhanced(symbol, tf, accountBalance = ACCOUNT_BALANCE) {
  ...
  try {
    console.log(`🔍 Starting analysis for ${symbol} ${tf}...`);
    const c = await candles(symbol, tf, tf.includes("m") ? 500 : 200);
    
    if (!c.length || c.length < 30) {
      console.warn(`Insufficient data for ${symbol} ${tf}: ${c.length} candles`);
      return { 
        success: false,
        direction: null, 
        quality: 0, 
    ...
    console.log(`✅ Data fetched: ${c.length} candles for ${symbol} ${tf}`);
    ...
    const signalKey = `${symbol}_${tf}_${Date.now()}`;
    SIGNAL_HISTORY.set(signalKey, signal);
...
<i>Synthetic Timeframes: 2D, 3D (generated from 1D data)</i>
    `.trim();
  ...
  if (cmd === "/analyze") {
    ...
    if (!symbol) {
      return send(chatId, 
        ...
        "Example: <code>/analyze BTCUSDT 4h</code>\n" +
        "Timeframes: 5m, 15m, 30m, 1h, 2h, 4h, 1d, 2d, 3d, 1w"
      );
    }
    ...
    if (decision.success) {
      ...
      const timeframeLabel = tf === "2d" || tf === "3d" ? `${tf} (Synthetic)` : tf;
      
      const signalMsg = `
<b>${arrow} ${directionText} SIGNAL</b>
<code>${symbol} • ${timeframeLabel} • ${new Date().toLocaleTimeString()}</code>
...
${tf === "2d" || tf === "3d" ? "<i>⚠️ Note: Using synthetic timeframe data</i>" : ""}
      `.trim();
  ...
  if (cmd === "/swing" || cmd === "/scalp") {
    ...
    for (const tf of tfs) {
      ...
      if (decision.success && decision.quality >= 60) {
        ...
        const timeframeLabel = tf === "2d" || tf === "3d" ? `${tf}*` : tf;
        
        results.push(
          `<b>${timeframeLabel}</b> ${arrow} ${decision.direction}\n` +
    ...
    const footer = cmd === "/swing" ? "\n<i>* Synthetic timeframe (generated from 1D data)</i>" : "";
...
║      Professional Trading System with Synthetic TFs      ║
╚══════════════════════════════════════════════════════════╝
  `);
  ...
  console.log("🔧 Synthetic Timeframes: 2D, 3D enabled");
  
  // Test data fetching on startup
  console.log("🧪 Testing data connection...");
  try {
    const testData = await candles("BTCUSDT", "1h", 10);
    if (testData.length > 0) {
      console.log(`✅ Data connection working: Fetched ${testData.length} BTCUSDT candles`);
      
      // Test synthetic timeframe
      console.log("🧪 Testing synthetic timeframe generation...");
      const syntheticTest = await candles("BTCUSDT", "2d", 10);
      if (syntheticTest.length > 0) {
        console.log(`✅ Synthetic timeframe working: Generated ${syntheticTest.length} 2D candles`);
      }
    } else {
      console.warn("⚠️ Could not fetch test data. Check internet connection or Binance API.");
    }
  } catch (error) {
    ...
    try {
      for (const [symbol, config] of WATCH) {
        ...
        if (decision.success && decision.quality >= 75) {
          ...
          console.log(`🚨 ${arrow} High-Quality Signal: ${symbol} ${config.tf} ${decision.direction} @ ${decision.entry}`);
          
          const alertMsg = `
<b>🚨 HIGH-CONVICTION SIGNAL</b>
<code>${symbol} • ${config.tf} • Auto-detected</code>

<b>Signal:</b> ${arrow} ${decision.direction === "BUY" ? "STRONG BUY" : "STRONG SELL"}
<b>Quality:</b> <code>${decision.quality}%</code>

<b>Entry:</b> <code>${decision.entry}</code>
<b>Stop Loss:</b> <code>${decision.stopLoss}</code>
<b>Take Profit 1:</b> <code>${decision.takeProfits[0]}</code>

<b>Risk:</b> ${decision.riskPercent}% of account
...
<i>Session: ${decision.session} | ATR: ${decision.atrPercent}%</i>
  ...
  console.log("🔧 Synthetic Timeframes: 2D, 3D (from 1D data)");
