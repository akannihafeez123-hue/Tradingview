...
class BitgetAPI {
  ...
  async makeRequest(endpoint, method = 'GET', params = {}, isFutures = false) {
    try {
      ...
      const signature = this.generateSignature(timestamp, method, endpoint, method === 'POST' ? JSON.stringify(params) : '');
  ...
  async getKlines(symbol, timeframe = '1H', limit = 200) {
    ...
    // CRITICAL FIX: Convert timeframe to Bitget format
    const bitgetTimeframe = BITGET_TF_MAP[timeframe] || "1H";
    const params = {
      symbol: symbol,
...
async function fetchCandles(symbol, tf, limit = 200) {
  ...
  try {
    ...
    const bitgetTf = BITGET_TF_MAP[tf] || "1H";
    
    let data;
    if (MARKET_TYPE === "futures") {
      data = await bitgetAPI.getFuturesKlines(symbol, bitgetTf, limit);
    } else {
      data = await bitgetAPI.getKlines(symbol, bitgetTf, limit);
    }
    
    if (!data || !data.data || !Array.isArray(data.data) || data.data.length === 0) {
      console.warn(`No candle data from Bitget for ${symbol} ${tf}`);
      return this.generateSyntheticCandles(symbol, tf, limit);
    }
